...
void runTests() {
  testWidgets('testInitialCenterLocationAtCenter', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MediaQuery(
        data: MediaQueryData(devicePixelRatio: 2.0), // Set to your preferred pixel ratio
    ...
    final ScreenCoordinate coordinate = await mapController.getScreenCoordinate(kInitialCameraPosition.target);
    final Rect rect = tester.getRect(find.byKey(key));
    final double devicePixelRatio = MediaQuery.of(tester.element(find.byKey(key))).devicePixelRatio;

    if (isIOS || isWeb) {
      expect(coordinate.x, (rect.center.dx - rect.topLeft.dx).round());
      expect(coordinate.y, (rect.center.dy - rect.topLeft.dy).round());
    } else {
      expect(coordinate.x, ((rect.center.dx - rect.topLeft.dx) * devicePixelRatio).round());
      expect(coordinate.y, ((rect.center.dy - rect.topLeft.dy) * devicePixelRatio).round());
    }
  ...
  testWidgets('testGetVisibleRegion', (WidgetTester tester) async {
    ...
    final LatLngBounds firstVisibleRegion = await waitForValueMatchingPredicate<LatLngBounds>(
            tester, () => mapController.getVisibleRegion(), (LatLngBounds bounds) => bounds != zeroLatLngBounds) ??
        zeroLatLngBounds;
  ...
  testWidgets('testToggleInfoWindow', (WidgetTester tester) async {
    ...
    iwVisibleStatus = await waitForValueMatchingPredicate<bool>(
            tester, () => controller.isMarkerInfoWindowShown(marker.markerId), (bool visible) => visible) ??
        false;
...
/// animations).
Future<T?> waitForValueMatchingPredicate<T>(WidgetTester tester, Future<T> Function() getValue, bool Function(T) predicate,
